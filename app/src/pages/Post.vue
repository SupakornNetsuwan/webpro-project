<template>
  <div className="w-full flex min-h-screen flex-col bg-gray-1">
    <content-wrapper class="" :noPadding="true">
      <div
        class="flex justify-between mt-4 mx-4 md:mx-8 lg:mx-12 pt-4 md:pt-8 lg:pt-12"
      >
        <button
        ...
        <button
          :class="
            isFollow
              ? 'border-solid border-blue-primary bg-blue-primary text-white flex items-center'
              : 'border-solid border-blue-primary bg-white text-blue-primary flex items-center'
          "
      ...
      <img
        v-if="post.post_img"
        :src="post.post_img"
        class="w-full h-96 object-cover my-5"
      />
      <p
    ...
    loadPost() {
      try {
        Promise.all([
          ...
          getFollowingPost(),
          getSuggestPosts(3),
        ]).then(([getPostRes, getFollowingPostRes, getSuggestPostsRes]) => {
          this.post = getPostRes.data;
          this.author = getPostRes.data.author;
